//! Rollout コマンドのユースケース
//!
//! 現在のセッションに対して SIGUSR1 を送り、コンソールログを Part ファイルに
//! ロールオーバーさせる（aish シェル側の SIGUSR1 ハンドラと同等の動き）。
...
impl RolloutUseCase {
    ...
    pub fn run(&self, path_input: &PathResolverInput) -> Result<i32, Error> {
        ...
        self.rollout_console_log(session.session_dir().as_ref())
    }
    ...
    fn rollout_console_log(&self, session_dir: &Path) -> Result<i32, Error> {
        let Some(pid) = self.resolve_attached_pid(session_dir)? else {
            return Ok(0);
        };

        // SIGUSR1 を送信（コンソールログのフラッシュ + ロールオーバー）
        self.signal.send_signal(pid, libc::SIGUSR1)?;
        Ok(0)
    }
...
mod tests {
    ...
    impl TestAttachmentStore {
        ...
        fn detached() -> Self {
            Self {
                attachment: Mutex::new(Some(ShellAttachment::detached_from(None, "test-now"))),
            }
        }
    }
    ...
    #[test]
    fn rollout_sends_sigusr1_when_shell_is_attached() {
        let temp_dir = std::path::PathBuf::from("/tmp").join("aish_test_rollout_attached");
        if temp_dir.exists() {
        ...
        let usecase = RolloutUseCase::new(
            Arc::clone(&path_resolver),
            Arc::clone(&fs),
            Arc::clone(&signal) as Arc<dyn SignalPort>,
            Arc::new(TestAttachmentStore::attached(321)),
        );
    ...
    #[test]
    fn rollout_is_noop_when_shell_is_detached() {
        let temp_dir = std::path::PathBuf::from("/tmp").join("aish_test_rollout_detached");
        if temp_dir.exists() {
            let _ = std::fs::remove_dir_all(&temp_dir);
        }
        std::fs::create_dir_all(&temp_dir).unwrap();
        ...
        let usecase = RolloutUseCase::new(
            ...
            Arc::clone(&fs),
            Arc::clone(&signal) as Arc<dyn SignalPort>,
            Arc::new(TestAttachmentStore::detached()),
        );
        ...
        assert!(signal.sent.lock().expect("lock poisoned").is_empty());
        let _ = std::fs::remove_dir_all(&temp_dir);
    }
}
