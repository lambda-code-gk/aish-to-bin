use crate::domain::{PromptSourceKind, ResolvedPromptSource, SkillSpec, TaskName, TaskSpec};
...
impl StdPromptSourceResolver {
    ...
    fn find_task_root(&self, task_name: &TaskName) -> Result<Option<(PathBuf, TaskKind)>, Error> {
        ...
        for loc in locations {
            if let Some(kind) = Self::task_kind_in_dir(self.fs.as_ref(), &loc.path, task_name) {
                return Ok(Some((loc.path.clone(), kind)));
            }
        }
        Ok(None)
    }
    ...
    fn find_task_origin(
        &self,
        task_name: &TaskName,
    ) -> Result<Option<(PathBuf, TaskKind, Option<crate::domain::ResolvedPackage>)>, Error> {
        // 1. 既存の task.d 由来
        if let Some((root, kind)) = self.find_task_root(task_name)? {
            return Ok(Some((root, kind, None)));
        }
        ...
        for pkg in pkgs {
            ...
            if let Some(kind) =
                Self::task_kind_in_dir(self.fs.as_ref(), &task_root, task_name)
            {
                return Ok(Some((task_root, kind, Some(pkg))));
            }
        }
        Ok(None)
    }
    ...
    fn resolve_package_system_hook(
        ...
        Some(ResolvedPromptSource {
            ...
            content: trimmed.to_string(),
            // package の memory_topics を最初に足すことで、タスク / skill の topics と合成される。
    ...
    fn resolve_skill_prompt_for(
        &self,
        skill_name: &str,
    ) -> Result<Option<(SkillSpec, String)>, Error> {
        ...
        for loc in locations {
            match self.skill_specs.load_skill_spec(&loc.path, skill_name)? {
                Some(spec) => {
                    ...
                    return Ok(Some((spec, trimmed.to_string())));
                }
                None => {}
            }
        }
        ...
        for pkg in pkgs {
            ...
            match self
                .skill_specs
                .load_skill_spec(&skills_root, skill_name)?
            {
                Some(spec) => {
                    let content = match self.fs.read_to_string(&spec.instructions_path) {
                        Ok(s) => s,
...
impl PromptSourceResolver for StdPromptSourceResolver {
    fn resolve_for_task(
        ...
        let (task_root, task_kind, package) = match origin {
            Some(v) => v,
            None => {
                // タスク自体が存在しない場合は hooks のみ返す。
                return Ok(hooks);
            }
        };
        ...
        let mut out = hooks;
        // package 由来タスクで package system hook があれば、task prompt より前に挿入する。
        ...
        if let Some(task_prompt) =
            self.resolve_task_prompt(&task_root, task_name, task_kind, spec_ref)
        {
...
mod tests {
    ...
    fn hooks_only_when_no_task() {
        ...
        let skill_specs: Arc<dyn SkillSpecLoader> =
            Arc::new(StubSkillSpecLoader { specs: vec![] });
        let packages: Arc<dyn crate::ports::outbound::PackageResolver> =
        ...
        let sources = resolver.resolve_for_task(&name, None).unwrap();
    ...
    fn hooks_task_prompt_and_skills_are_composed_in_order() {
        ...
        let sources = resolver.resolve_for_task(&task_name, None).unwrap();
