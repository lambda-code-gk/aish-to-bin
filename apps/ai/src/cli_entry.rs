//! 責務: ai CLI entry と UseCaseRunner の橋渡しを行う。
...
use crate::cli::{config_to_command, print_completion, Config, ParseOutcome};
...
impl Runner {
    ...
    fn run_policy_explain(&self) -> Result<i32, Error> {
        ...
        let json = serde_json::to_string_pretty(&info).map_err(|e| Error::json(e.to_string()))?;
        println!("{}", json);
        Ok(0)
    }
    ...
    fn run_sessions_rebuild_derived(&self, session_dir: Option<SessionDir>) -> Result<i32, Error> {
    ...
    fn run_query_cmd(
        ...
        if query.trim().is_empty() {
            return Err(Error::invalid_argument(
                "No query provided. Use -c or --continue to resume a previous session.".to_string(),
            ));
        }
        let max_turns = std::env::var("AI_MAX_TURNS")
            .ok()
...
fn log_verbose_snapshot(
    ...
    if message_args_len > 0 {
        m.insert(
            "message_args_len".to_string(),
            serde_json::json!(message_args_len),
        );
    }
    let _ = logger.log(&LogRecord {
        ts: now_iso8601(),
...
pub fn run() -> Result<i32, Error> {
    ...
    crate::backend_dispatch::run_with_invocation_args(args, run_with_outcome)
}
...
fn print_help() {
    ...
    println!(
        "  -v, --verbose                 Emit verbose debug logs to stderr (for troubleshooting)."
    );
    ...
    println!(
        "  --list-tasks                   List available task names (used by shell completion)."
    );
    println!(
        "  --list-modes                   List available mode names (used by shell completion)."
    );
    println!();
    println!("Environment:");
