//! Library entry for ai: run from env args or from given args (used by crates/aish).
...
mod entry {
    ...
    impl Runner {
        ...
        fn run_list_tools(
            &self,
            profile: Option<common::domain::ProviderName>,
        ) -> Result<i32, Error> {
    ...
    /// Run ai with arguments from the environment (binary entry point).
    pub fn run() -> Result<i32, Error> {
        let outcome = parse_args()?;
        run_with_outcome(&outcome)
    }

    /// Run ai with the given argument iterator (used by crates/aish for `aish ai ...`).
